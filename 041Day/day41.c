// 0x41Day of 0x365Days challenge
// VEDANT BHARAD
// 27-11-2022
#include <assert.h>
#include <limits.h>
...
int getMoneySpent(int keyboards_count, int* keyboards, int drives_count, int* drives, int b) {
    int moneySpent = 0;
    for(int loop1=0; loop1 < keyboards_count; loop1++) {
        for(int loop2=0; loop2 < drives_count; loop2++){
            if((keyboards[loop1]+drives[loop2]<=b) && (keyboards[loop1]+drives[loop2]>moneySpent)){
                moneySpent = keyboards[loop1]+drives[loop2];
            }
        }
    }
    if(moneySpent==0){
